...
public class RunArgumentsNew implements RunArguments
{
	...
	@Override
	public List<String> build(LoadedVersion version, String classpath, String nativesDirectory, DataBaseOfUser profile)
	{
		...
		for(int i = 0; i < game.size(); i++)
		{
			...
			if(m.find())
			{
				...
				else if ("clientid".equals(found))
				{
					replacement = "defaultClientId"; // Vous devez ajouter cette méthode dans DataBaseOfUser si elle n'existe pas.
				}
				else if ("auth_xuid".equals(found))
				{
					replacement = "auth_xuid"; // Ajoutez un getter pour XUID dans DataBaseOfUser si nécessaire.
				}
		...
		//System.out.println("Classpath final : " + classpath);
